...
```{r, echo=F}
knitr::include_url('https://data.census.gov/vizwidget?g=010XX00US&infoSection=Age and Sex')
knitr::include_url('https://data.census.gov/vizwidget?g=040XX00US36&infoSection=Age and Sex')
```
...
turnout_data <- read.csv('Turnout_1980_2022_v1.1.csv')

ny_turnout <- turnout_data %>% filter(STATE=="New York", YEAR %% 4 == 0) %>%
                mutate(VEP_TURNOUT_RATE=as.numeric(strsplit(VEP_TURNOUT_RATE, '%'))) %>% add_row(YEAR=2024, VEP_TURNOUT_RATE=57.87, .before=TRUE)

ggplot(data=ny_turnout, aes(x=YEAR, y=VEP_TURNOUT_RATE)) + 
  geom_point() + 
  geom_line() +
  ggtitle("NYS Turnout in Presidential Elections Since 1980")
...
# sources: 538 and elections2024.thehill.com

library(gt)
library(dplyr)
library(rmarkdown)

ratings <- data.frame(c("fivethirtyeight", "Siena College", "Research Co.", "Cygnal", "ActiVote", "YouGov/Claremont McKenna"), c("58.5%", "58.0%", "57.0%", "56.8%", "59.0%", "58.0%"))
colnames(ratings) <- c("Source", "Democratic Vote Share Prediction in New York")
