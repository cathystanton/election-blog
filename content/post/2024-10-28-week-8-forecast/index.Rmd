...
#                group_by(state, year) %>%
#                nest() %>%
#                mutate(correlation = purrr::map(ec_results_scaled, ~ correlate(.x %>% #select(-Group))),
#                       pca = purrr::map(ec_results_scaled, ~ princomp(.x %>% #select(-Group), cor = TRUE)))
